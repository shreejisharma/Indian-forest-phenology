...
def add_derived_features(met_df, season_start_month=1):
    """
    Compute derived features from whatever columns are present in met_df.
    Fully adaptive — no column names assumed beyond checking presence.
    """
    df = met_df.copy()
    ...
    if tavg is not None:
        ...
        def _season_cumsum(series, dates, sm_):
            out = series.copy() * 0.0
            season_yr = dates.apply(lambda d: d.year if d.month >= sm_ else d.year - 1)
...
def get_all_correlations(X, y):
    ...
    for col in X.columns:
        vals = X[col].dropna()
        if vals.std() < 1e-8 or len(vals) < 3:
            continue
...
def select_multi_features(X, y, max_features=5, min_r=MIN_CORR_THRESHOLD):
    """
    Purely data-driven feature selection:
    1. Filter by Pearson/Spearman composite ≥ min_r
    2. Remove collinear features (|r_pair| > 0.85)
    3. Forward selection by LOO R² improvement
    No event priority lists — features are ranked by data correlation only.
    """
    n_obs  = len(y.dropna())
    usable = []
    for col in X.columns:
...
from your data's own correlations.
    ...
    with tab2:
        ...
        if n_seasons < 3:
            st.error(f"❌ Only {n_seasons} season(s) detected — need ≥ 3 for modelling."); return
        st.success(f"✅ **{n_seasons} growing seasons** extracted from data")
        ...
        if n_seasons < 7:
            st.markdown(f'<div class="warn-box">⚠️ <b>{n_seasons} seasons</b> — small training set. '
                        f'R² 0.3–0.6 is realistic. Collect more years for stronger models.</div>',
                        unsafe_allow_html=True)
        else:
            st.markdown('<div class="good-box">✅ Good season count — model estimates are reliable.</div>',
                        unsafe_allow_html=True)
